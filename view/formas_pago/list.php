...
<html>
  ...
  <body class="hold-transition <?php echo $skin;?> sidebar-mini">
	<?php 
		...
		include("modal/agregar_forma_pago.php");
		include("modal/editar_forma_pago.php");
		}
	?>  
    <div class="wrapper">
	...
	function per_page(valor){
		...
		function eliminar(id){
			if(confirm('Esta acción  eliminará de forma permanente la forma de pago \n\n Desea continuar?')){
				var page=1;
				var query=$("#q").val();
				var per_page=$("#per_page").val();
...
$( "#new_register" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/registro/agregar_forma_pago.php",
			data: parametros,
			 beforeSend: function(objeto){
...
$( "#update_register" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/modificar/forma_pago.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados_ajax").html("Enviando...");
			  },
...
<script>
  ...
  modal.find('.modal-body #label_edit').val(label)
  modal.find('.modal-body #days_edit').val(days)
