...
<html>
  ...
  <body class="hold-transition <?php echo $skin;?> sidebar-mini">
	<?php 
		...
		include("modal/agregar_tiraje.php");
		include("modal/editar_tiraje.php");
		}
	?>  
    <div class="wrapper">
        ...
        <section class="content-header">
				<div class="row">
					...
					<div class="input-group">
						<select name="branch_id" id="branch_id" class="form-control" onchange="load(1);">
	...
	function per_page(valor){
		...
		function eliminar(id){
			if(confirm('Esta acción  eliminará de forma permanente el tiraje del documento \n\n Desea continuar?')){
				var page=1;
				var query=$("#q").val();
				var branch_id=$("#branch_id").val();
				var per_page=$("#per_page").val();
				var parametros = {"action":"ajax","page":page,"query":query,'branch_id':branch_id,"per_page":per_page,"id":id};
...
$( "#new_register" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/registro/agregar_tiraje.php",
			data: parametros,
			 beforeSend: function(objeto){
...
<script>
  ...
  var inicio = button.data('initial')
  var fin = button.data('final')
  var type_document= button.data('type_document')
  var branch_id= button.data('branch_id')
  var id= button.data('id')
  var modal = $(this)

  modal.find('.modal-body #code2').val(code)
  modal.find('.modal-body #initial2').val(inicio)
  modal.find('.modal-body #final2').val(fin)
  modal.find('.modal-body #type_document2').val(type_document)
