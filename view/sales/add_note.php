...
<html>
  ...
  <body class="hold-transition <?php echo $skin;?> sidebar-mini">
						...
						<form method="post" name="venta" id="venta">
                            <div class="row">
									...
									<div class="col-md-3">
										...
										<select class="form-control" id="apply_to" name="apply_to" onchange="add_tmp(this.value);" >
											...
											?>
											<option value="<?php echo $rw['sale_id']?>" ><?php echo $rw['sale_prefix']." ".$rw['sale_number']." | Cliente: ".$rw['name'];?></option>
											<?php
												}
											?>
										</select>
	...
	$(function () {
		...
		$(".select2").select2();
		$( "#resultados" ).load( "./ajax/agregar_nc_tmp_sale.php" );
		load(1);
	});
		function load(page){
			var q= $("#q").val();
	...
	function agregar (id)
		{
			...
			$.ajax({
        type: "POST",
        url: "./ajax/agregar_nc_tmp_sale.php",
...
$( ".guardar" ).click(function() {
  ...
  if (payment_method>1 &&  customer_id==""){
   ...
   $.ajax({
        type: "POST",
        url: "./ajax/registro/nota_credito.php",
        data: parametros,
		 beforeSend: function(objeto){
			$("#loading_text").show();
			$(".datos_venta").html("");
...
<script>
	function taxes(value){
		$( "#resultados" ).load( "./ajax/agregar_nc_tmp_sale.php?taxes="+value );
	}
</script>
<script>
	function currency(value){
		$( "#resultados" ).load( "./ajax/agregar_nc_tmp_sale.php?currency_id="+value );
	}
</script>

<script>
	function add_tmp(value){
		$( "#resultados" ).load( "./ajax/agregar_nc_tmp_sale.php?apply_to="+value );
	...
	function update_tmp(id, campo, valor){
		$( "#resultados" ).load( "./ajax/agregar_nc_tmp_sale.php?id_tmp="+id+"&campo="+campo+"&valor="+valor );
		//alert(value);
