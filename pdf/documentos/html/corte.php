 ...
 <?php echo $F." ".$hora;?>
	...
	<table class="border" >
		...
		<?php
						...
						$valor_entregar=$fondo_fijo_ingresos-$total_egresos-$fondo_fijo;
		?>
		<tr>
		...
		<tr>
			...
			<td style="width:15%;text-align: right"></td>
			<td style="width:15%;text-align: right"></td>
			<td style="width:15%;text-align: right"><strong><?php echo number_format($fondo_fijo,$precision_moneda,$sepador_decimal_moneda,$sepador_millar_moneda);?></strong></td>
		</tr>
		<tr>
...
<?php 
	...
	$balance=$valor_entregar+$get_balance;
	$created_at=date("Y-m-d H:i:s");
	save_finances("Corte de caja $nombre_sucursal",1,$valor_entregar,$balance,$created_at,$user_id,1);
	$id_finances=get_last_id('finances','id');//Obtengo el id de la finances ingresado
	$id_corte=get_last_id('cashier_closing','id');//Obtengo el ultimo del corte ingresado
	update_transacion_id($id_corte,$id_finances,'cashier_closing','id');//Actualiza el id del corte para vincular a tabla finances
?>
