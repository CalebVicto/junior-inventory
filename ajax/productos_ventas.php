	...
	if($action == 'ajax'){
		// escaping, additionally removing everything that could be (html/javascript-) code
		 ...
		 if ($is_service==0){
			...
			$sWhere .=" and inventory.branch_id='$id_sucursal'";
			...
			$aColumns = array('product_code', 'product_name');//Columnas de busqueda
			 $sTable = "products";
			 $sWhere = "";
			 $sWhere .= " where products.is_service=1";
			 if ( $_GET['q'] != "" )
			{
				$sWhere .= " and (products.product_code LIKE '%".$q."%' OR products.product_name LIKE '%".$q."%')";
			}
		...
		if ($numrows>0){
			...
			<div class="table-responsive">
			  <table class="table">
				<tr  class="warning">
					<th>Código</th>
					<th><?php if ($is_service==0){echo "Producto";}else {echo "Servicio";}?></th>
					<?php if ($is_service==0){?>
					<th>Fabricante</th>
				...
				while ($row=mysqli_fetch_array($query)){
					...
					<tr>
						...
						<td class='text-center'><?php echo get_stock($id_producto,$id_sucursal); ?></td>
						<?php }?>
						<td class='col-xs-1'>
						...
						<td class='col-xs-1'>
							<div class="pull-right">
								<input type="number" class="form-control" style="text-align:right" id="descuento_<?php echo $id_producto; ?>"   >
							</div>
						</td>
