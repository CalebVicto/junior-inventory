...
if (isset($_POST['barcode'])){
	...
	if ($product_id>0){
		...
		add_tmp($product_id, $barcode_qty, $unit_price, $user_id,0,$id_sucursal);
	} else {
		echo "<script>alert('Stock insuficiente.')</script>";
	}
	} else {
...
if (isset($_POST['id_guia'])){
	...
	guiaToInvoice($id_guia,$user_id, $branch_id);//Convierte la guia en Factura
	//$update=mysqli_query($con,"update referral_guides set status='1' where id='$id_guia'");
	//$update=mysqli_query($con,"update referral_guides set comprobante='$number_document' where id='$id_guia'");
	add_tmp_guia($id_guia, $user_id);//Agrego a la tabla temporal el id de la guia
}
