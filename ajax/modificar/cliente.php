		...
		if (empty($_POST['client_id'])){
			...
			// escaping, additionally removing everything that could be (html/javascript-) code
				...
				$client_id=intval($_POST['client_id']);//ID del cliente
                    ...
                    $sql = "UPDATE customers, contacts SET customers.name='$name', customers.tax_number='$tax_number', customers.website='$website', customers.work_phone='$work_phone', contacts.first_name='$first_name', contacts.last_name='$last_name', contacts.email='$email', phone='$phone', address1='$address1', city='$city', state='$state', postal_code='$postal_code', country_id='$country_id', discount_rate='$discount_rate'  WHERE customers.id=contacts.client_id and customers.id='".$client_id."'";
                    $query = mysqli_query($con,$sql);

                    // if  has been update successfully
                    if ($query) {
                        $messages[] = "El cliente ha sido actualizado con éxito.";
						save_log('Clientes','Actualización de datos',$_SESSION['user_id']);
