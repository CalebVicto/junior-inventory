<?php
	...
	while ($rw=mysqli_fetch_array($sql)){
		...
		$qty=$rw['qty'];
		add_inventory($product_id,$qty,$id_sucursal);//agrega a los productos al inventario
	...
	if($delete=mysqli_query($con, "DELETE FROM credit_notes WHERE id='".$note_id."'") ){
		...
		//Ahora se debe realizar un abono al la cuenta por pagar con signo negativo porque se elimino la nota de credito
		...
		$insert=mysqli_query($con,"INSERT INTO payments (payment_id, purchase_id, total, payment_date, payment_type, number_reference, note, user_id) 
			VALUES (NULL, '$apply_to', '-$total_venta', '$payment_date', '4', '', '', '$user_id');");
		//$insert=mysqli_query($con,"INSERT INTO charges (charge_id, sale_id, total, payment_date, payment_type, number_reference, note, user_id) VALUES (NULL, '$apply_to', '-$total_venta', '$payment_date', '1', '', '', '$user_id');");
...
if($action == 'ajax'){
	...
	$sWhere="users.user_id=credit_notes.sale_by and credit_notes.transaction_type=1";
	...
	if ($supplier_id>0){
		$sWhere.=" and credit_notes.customer_id = '".$supplier_id."'";
	}
				...
				<div class="table-responsive">
					<table class="table table-condensed table-hover table-striped ">
						...
						<tr>
							...
							<td><?php echo $simbolo_moneda;?> <span class='pull-right'><?php echo number_format($total,$precision_moneda,$sepador_decimal_moneda,$sepador_millar_moneda);?></div></td>
							...
							<div class="btn-group pull-right">
								...
								<ul class="dropdown-menu">
									...
									<li><a href="edit_note_buy.php?id=<?php echo $note_id;?>"><i class='fa fa-eye'></i> Ver detalles</a></li>
									<li><a href="#" onclick="note_print('<?php echo $note_id;?>')"><i class='fa fa-print'></i> Imprimir</a></li>
									<?php }
