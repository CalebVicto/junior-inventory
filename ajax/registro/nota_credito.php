...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
	...
	if (empty($_POST['apply_to'])) {
			...
			// escaping, additionally removing everything that could be (html/javascript-) code
				...
				$customer_id = get_id("sales","customer_id","sale_id",$apply_to);
				...
				if ($count_tmp>0){
					...
					$add_note= add_note($number_document, $sale_prefix, $customer_id, $user_id,$sale_date, $branch_id,$seller_id,$cashbox_id,$includes_tax,$apply_to,$transaction_type,$currency_id);
					if ($add_note==1){
						 $messages[] = "La nota de crédito ha sido guardada con éxito.";
						 save_log('Notas de crédito','Registro de nota de crédito venta',$_SESSION['user_id']);
...
if (isset($errors)){
			...
			if (isset($messages)){
				...
				<center>
					<a href="note-print-pdf-sale.php?id=<?php echo $next_insert_id;?>" target="_blank" class="btn btn-success" style="margin-right: 5px;"><i class="fa fa-download"></i> Generar PDF</a>
				</center>
				<?php
			}
?>			