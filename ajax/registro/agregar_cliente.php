...
if (empty($_POST['bussines_name'])){
			...
			// escaping, additionally removing everything that could be (html/javascript-) code
				...
				$discount_rate	 = mysqli_real_escape_string($con,(strip_tags($_POST["discount_rate"],ENT_QUOTES)));
				
				$created_at=date("Y-m-d H:i:s");
               
				if (!empty($tax_number)){
					$sql_customer=mysqli_query($con,"select * from customers where tax_number='$tax_number'");
				...
				if ($count==0){
                    ...
                    $sql = "INSERT INTO customers (created_at,name,address1,city,state,postal_code,	country_id, work_phone, website, tax_number,discount_rate) VALUES('$created_at','$name','$address1','$city','$state','$postal_code', '$country_id','$work_phone','$website','$tax_number','$discount_rate');";
                    ...
                    if ($query) {
						...
						save_log('Clientes','Registro de cliente',$_SESSION['user_id']);
					} else {
