<?php
	...
	$employee_id=intval($_REQUEST['sale_by']);
	$status=mysqli_real_escape_string($con,(strip_tags($_REQUEST['status'], ENT_QUOTES)));
	$tables="quotes, customers, users";
	$campos="*";
	$sWhere="quotes.customer_id=customers.id and users.user_id=quotes.employee_id";
	if ($employee_id>0){
		$sWhere.=" and quotes.employee_id = '".$employee_id."'";
	}
	if ($status!="todas"){
	...
	require_once(dirname(__FILE__).'/pdf/html2pdf.class.php');
     ...
     include(dirname('__FILE__').'/pdf/documentos/html/quotes-report.php');
